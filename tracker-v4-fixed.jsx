...
// ══ DB + API ══════════════════════════════════════════════════
const db={get:async(k)=>{try{const r=await window.storage.get(k);return r?JSON.parse(r.value):null;}catch{return null;}},set:async(k,v)=>{try{await window.storage.set(k,JSON.stringify(v));}catch(e){console.error(e);}}};
async function ai(msg,sys="",hist=[],mcpServers=[]){const body={model:"claude-sonnet-4-20250514",max_tokens:1200,messages:[...hist.map(m=>({role:m.role,content:m.content})),{role:"user",content:msg}]};if(sys)body.system=sys;if(mcpServers.length)body.mcp_servers=mcpServers;const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const d=await r.json();return d.content?.find(c=>c.type==='text')?.text||"No response.";}
async function aiVision(imageB64,mediaType,promptText,sys=""){const body={model:"claude-sonnet-4-20250514",max_tokens:1200,system:sys,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:mediaType,data:imageB64}},{type:"text",text:promptText}]}]};const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const d=await r.json();return d.content?.[0]?.text||"Could not analyze image.";}
